...
import { 
  Shield, CheckCircle, ArrowRight, Target, FileText, Brain, DollarSign, 
  Users, Clock, Award, AlertTriangle, Lightbulb, BarChart3, Eye, 
  Clipboard, BadgeCheck, Calculator, FileSearch, ScanLine, Network, Car, Home, 
  HeartHandshake, Building, Plane, Briefcase, AlertCircle
} from 'lucide-react';
...
export default function InsuranceIndustrySection() {
  ...
  const t = translations[language].insuranceIndustry;
  ...
  const keyBenefits = t.hero.keyBenefits.map((text, index) => ({
    icon: [FileText, Eye, BarChart3, Network][index],
    text: text
  }));

  const insuranceSolutions = t.solutions.items.map((item, index) => ({
    ...item,
    icon: [FileText, Eye, BarChart3][index],
    link: index === 2 ? '/solutions/ai-doc' : '/solutions/ai-data'
  }));
  ...
  const insuranceCapabilities = t.capabilities.items.map((item, index) => ({
    ...item,
    icon: [FileText, Eye, BarChart3, Clipboard, ScanLine, FileSearch, Target, BadgeCheck, Calculator][index]
  }));

  const insuranceLines = t.lines.items.map((item, index) => ({
    ...item,
    icon: [Car, Home, HeartHandshake, Briefcase, Building, Plane][index]
  }));
                ...
                >
                  <div className="relative bg-white/90 backdrop-blur-sm rounded-3xl p-8 shadow-2xl border border-ai-light/50">
                    <div className="grid grid-cols-3 gap-4 mb-6">
                      {[
                        { icon: Shield, color: 'text-blue-600' }, { icon: Car, color: 'text-ai-green' },
                        { icon: Home, color: 'text-blue-600' }, { icon: HeartHandshake, color: 'text-ai-green' },
                        { icon: Eye, color: 'text-blue-600' }, { icon: FileText, color: 'text-ai-green' },
                        { icon: BarChart3, color: 'text-blue-600' }, { icon: Calculator, color: 'text-ai-green' },
                    ...
                    <div className="bg-gradient-to-r from-blue-50 to-green-50 rounded-xl p-4 mb-6">
                      <div className="flex items-center justify-between text-center">
                        <div>
                          ...
                          <div className="text-xs text-ai-medium">{t.visual.claimFiled}</div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="text-xs text-ai-medium">{t.visual.settled}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="text-center p-3 bg-white rounded-lg shadow-sm">
                        <div className="text-2xl font-bold text-blue-600 mb-1">{t.visual.processingTime}</div>
                        <div className="text-xs text-ai-medium">{t.visual.processingLabel}</div>
                      </div>
                      <div className="text-center p-3 bg-white rounded-lg shadow-sm">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-green-50 rounded-2xl p-8 mb-16">
              <div className="text-center mb-8">
                <h3 className="text-2xl font-bold text-ai-dark mb-4">{t.lines.title}</h3>
                <p className="text-ai-medium">{t.lines.subtitle}</p>
              </div>
          ...
          <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
              {t.finalCta.trustIndicators.map((stat, index) => {
                const Icon = [FileText, Eye, BarChart3, Users][index];
                return (
